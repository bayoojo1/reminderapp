...
?><?php
    if (isset($_POST['action']) && $_POST['action'] == "share"){
        ...
        $row = mysqli_fetch_row($query);
        $data = "<br /><b>Shared by $row[0]</b><br /><hr><br />";
        $data .= $row[1];
        $sql = "INSERT INTO status(account_name, author, type, data, postdate) VALUES('$log_username','$log_username','a','$data',now())"; $query = mysqli_query($db_connect, $sql);
        mysqli_close($db_connect);
        echo "share_ok";
        exit();
    }
