...
if(isset($_POST['numeralid'])) {
    $feed_tag = lcfirst(preg_replace('#[^a-z]#i', '', $_POST['tag']));
    $feed_id = preg_replace('#[^0-9]#i', '', $_POST['numeralid']);
    $recipientmobile = preg_replace('#[^0-9,]#i', '', $_POST['content']);
}
...
$stmt->bindParam(':id', $feed_id, PDO::PARAM_STR);
$stmt->execute();

foreach($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
    $audio = $row['audio'];
    $e = $row['email']; // Do I need this?
    $provider = $row['firstname']; // Do I need this?
}
...
<body style="margin:0px; font-family:Tahoma, Geneva, sans-serif;">
  <div style="padding:10px; background:#004080; font-size:24px; color:#fff; text-align:center;"><a href="http://www.naatcast.com"><img src="cid:logo_2u" width="36" height="30" alt="NaatCast" style="border:none; float:left;"></a>NaatCast - Audio Request Notification
  </div>
...
$detail = 'rebroadcast audio content';
$action = 'rebroadcast';
