...
$sql = "SELECT * FROM monthly WHERE postdate >= DATE_SUB(NOW(),INTERVAL 1 MONTH)";
...
if($numrows == 0) {
  ...
  $sql = "UPDATE monthly SET `accepted` = DATE_ADD(`accepted` , INTERVAL 1 MONTH)";
  $stmt = $db_connect->prepare($sql);
  $stmt->execute();
}
// Create a view
$sql = "CREATE OR REPLACE VIEW `virtual` AS SELECT * FROM monthly";
$stmt = $db_connect->prepare($sql);
$stmt->execute();
