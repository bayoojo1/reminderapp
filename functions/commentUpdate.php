...
if(isset($_POST['post_id']) && isset($_POST['post_tag'])){
    $postid = preg_replace('#[^0-9]#i', '', $_POST['post_id']);
    $posttag = preg_replace('#[^a-z]#i', '', $_POST['post_tag']);
}else{
    exit();
}
...
foreach($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
    ...
    if($numrows > 5) {
    echo '<div class="load-comment" style="text-align:center; width:90px; margin:0 auto;">Load More</div>';
