...
$sql = "SELECT * FROM weekly WHERE postdate >= DATE_SUB(NOW(),INTERVAL 1 WEEK)";
...
if($numrows == 0) {
  ...
  $sql = "UPDATE weekly SET `accepted` = DATE_ADD(`accepted` , INTERVAL 1 WEEK)";
  $stmt = $db_connect->prepare($sql);
  $stmt->execute();
}
// Create a view
$sql = "CREATE OR REPLACE VIEW `virtual` AS SELECT * FROM weekly";
$stmt = $db_connect->prepare($sql);
$stmt->execute();
...
$sql = "SELECT username1, mobile, accepted, message, audio
        FROM `virtual`
        WHERE postdate >= DATE_SUB(NOW(),INTERVAL 1 WEEK)";
        $stmt = $db_connect->prepare($sql);
        $stmt->execute();
        $numrows = $stmt->rowCount();
        if($numrows == 0){
