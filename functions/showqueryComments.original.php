...
$sql = "SELECT tickets.ticket_id, ticket_detail, ticket_owner, ticket_date, commenter, queries.queryoption FROM tickets INNER JOIN queries ON queries.id=tickets.ticket_id WHERE ticket_id=:ticket_id AND queryoption=:queryoption AND commenter=:commenter OR commenter LIKE :naatcast ORDER BY tickets.id DESC";
$stmt = $db_connect->prepare($sql);
$stmt->bindParam(':ticket_id', $commentid, PDO::PARAM_INT);
$stmt->bindParam(':commenter', $commentuser, PDO::PARAM_STR);
