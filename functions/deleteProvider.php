...
// Delete the attached documents submitted by this provider and all provider detail
$sql = "SELECT provider, content_sample, identification FROM content_provider WHERE id=:id";
...
foreach($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
    ...
    $identity_doc = $row['identification'];
}

// Delete content_sample
$sample = basename($content_sample);
$sample_path = "../user/".$row['provider'].'/'.$sample;
unlink($sample_path);

// Delete identification
$identification = basename($identity_doc);
$id_path = "../user/".$row['provider'].'/'.$identification;
unlink($id_path);

// Delete the provider from database
$sql = "DELETE FROM content_provider WHERE id=:id";
$stmt = $db_connect->prepare($sql);
$stmt->bindParam(':id', $id, PDO::PARAM_INT);
$stmt->execute();
}
?><?php
// Return all these to the ajax
...
foreach($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
    ...
    if($approval == '1') {
    $subscription_List .= '<span style="font-weight:800; color:white;">Approval: </span>'.'<input id='.$row['provider'].' type="checkbox" class="approve" onclick="approved(this.id)" checked>';
    } else {
        $subscription_List .= '<span style="font-weight:800; color:white;">Approval: </span>'.'<input id='.$row['provider'].' type="checkbox" class="approve" onchange="approved(this.id)">';
    }
